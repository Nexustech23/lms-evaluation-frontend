  ...
  return (
      ...
      <div className="p-6 bg-white m-4 rounded-xl shadow-sm">
        <div className="flex justify-between mb-6">
          ...
          <button className="bg-green-600 px-2 py-2 rounded text-white " onClick={() => router.push(`/faculty/co/combinedCO?subjectId=${subjectId}`)}>{t("viewCombinedCO")}</button>
        </div>
            ...
            {exams.map((exam) => {
              ...
              return (
                <div key={exam.id}  className="flex flex-col cursor-pointer bg-blue-50 border border-blue-200 rounded-xl p-5 shadow-sm hover:shadow-md hover:bg-blue-100 transition-all duration-200">
                  ...
                  <div   style={{ backgroundColor: user?.color }} onClick={() => router.push(`/faculty/evaluate-answer-script/${exam.id}`)} className="text-sm text-gray-600 mt-2 bg-orange-500 text-white p-2 rounded flex justify-center">
                    {t("goToEvaluation")}
                  </div>
                </div>
              );
