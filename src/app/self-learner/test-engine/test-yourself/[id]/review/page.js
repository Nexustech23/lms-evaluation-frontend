...
export default function ReviewPage() {
    ...
    // Roadmap-mode MCQ answers are 0-indexed option positions, not text —
    // index 0 is a valid, common answer, so `|| ""` would wrongly discard
    // it (0 is falsy in JS); nullish coalescing preserves it.
    ...
    const isRoadmapMode = testInfo.mode === "roadmap";

    // For roadmap-mode MCQ, map an index back to its option text for
    // display purposes only (comparisons below stay index-based).
    const displayAnswer = (val) => {
        if (isRoadmapMode && options.length > 0 && typeof val === "number") {
            return options[val] ?? String(val);
        }
        return val;
    };
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border overflow-hidden mb-4"
                                ...
                                <div className="flex items-center gap-2 text-xs">
                                    {isRoadmapMode ? (
                                        <>
                                            ...
                                            <span className="font-bold" style={{ color: currentQW.score >= 50 ? "#22c55e" : "#ef4444" }}>
                                                {currentQW.score ?? 0}%
                                            </span>
                                        </>
                                    ) : (
                            ...
                            <div className="px-5 py-5">
                                <p className="text-gray-800 text-base font-medium leading-relaxed mb-5">
                                    {currentQW.questionText || currentQW.question || "Question text unavailable"}
                                </p>
                                ...
                                {options.length > 0 && (
                                    <div className="space-y-2.5 mb-5">
                                        {options.map((opt, i) => {
                                            ...
                                            const isCorrect = isRoadmapMode
                                                ? i === correctAns
                                                : opt.toLowerCase() === correctAns;
                                            const isChosen = isRoadmapMode
                                                ? i === studentAns
                                                : opt.toLowerCase() === studentAns;
                                            const isWrong = isChosen && studentAns !== correctAns;
                                            return (
                                                <OptionRow
                                                    key={i}
                                                    label={label}
                                    ...
                                    <div className="space-y-3 mb-5">
                                        <div className="rounded-xl p-3 border"
                                            ...
                                            <p className="text-sm text-red-700 font-medium">
                                                {displayAnswer(studentAns) || <span className="italic text-gray-400">Not attempted</span>}
                                            </p>
                                        </div>
                                        <div className="rounded-xl p-3 border"
                                            style={{ backgroundColor: "#f0fdf4", borderColor: "#22c55e" }}>
                                            <p className="text-xs text-green-600 font-semibold mb-1">
                                                {isRoadmapMode ? "Model Answer" : "Correct Answer"}
                                            </p>
                                            <p className="text-sm text-green-700 font-medium">{displayAnswer(correctAns)}</p>
                                        </div>
                                    </div>
                                )}
                                ...
                                <div className="flex flex-wrap gap-3 mb-5 text-xs">
                                    <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full"
                                        ...
                                        <span className="text-green-600">{displayAnswer(correctAns) || "—"}</span>
                                    </div>
                                    {qStatus !== "skipped" && (
                                        <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full"
                                            style={{
                                ...
                                {(currentQW.explanation || currentQW.feedback) && (
                                    <div className="rounded-xl p-4 border"
                                        style={{ backgroundColor: "#fffbeb", borderColor: "#fde68a" }}>
                                        <p className="text-xs font-bold text-amber-700 mb-1.5">
                                            💡 {isRoadmapMode ? "AI Feedback" : "Explanation"}
                                        </p>
