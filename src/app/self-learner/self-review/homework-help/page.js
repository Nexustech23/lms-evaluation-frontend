...
export default function Page() {
  ...
  const [homeworkType, setHomeworkType] = useState("Detailed Solution");
    ...
    pollTimerRef.current = setTimeout(async () => {
      try {
        const res = await axios.get(
          `/api/ai-tutor/homework-help/status/${jobId}`,
  ...
  const handleGenerate = async () => {
    try {
      if (!prompt && !file) {
        toast.error("Please enter homework details or upload a file.");
        return;
      }
      ...
      <div className="max-w-5xl mx-auto bg-white rounded-3xl border border-gray-200 shadow-sm p-5 md:p-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-600 mt-2 font-medium">
            Add homework instructions, upload PDFs, or describe what help you need.
          </p>
        </div>
...
Help with programming homework.`}
        ...
        >
          {loading ? "Generating Homework Help..." : "Generate Homework Help"}
        </button>
        ...
        {!loading && (
          <OutputPreview
            ...
            result={result}
            emptyMessage="Your AI-generated homework solution, explanations, summaries, and answers will appear here."
          />
        )}
      </div>
    </div>
